...
class JpaWebhookProviderIT {
    ...
    void storeEvent_idempotent_on_duplicate_kcEventId() {
        ...
        WebhookEventModel e1 =
                provider.storeEvent(
                        mockRealm, KeycloakEventType.USER, "kc-123", "{\"type\":\"access.LOGIN\"}");
        assertNotNull(e1);
    ...
    void storeSend_uniqueConstraint_prevents_duplicate_records() {
        ...
        WebhookSendModel s1 =
                provider.storeSend(
                        mockRealm, w.getId(), event.getId(), "access.LOGIN", 200, true, 0);
        assertNotNull(s1);
