...
/**
 * Custom Keycloak SPI providing data access for webhooks, events, and send records. Implementations
 * handle CRUD operations, event persistence, and delivery audit trail.
 *
 * <p>All methods accept a {@link RealmModel} for tenant scoping. The JPA implementation uses
 * Keycloak's existing datasource and entity manager.
 *
...
public interface WebhookProvider extends Provider {
    ...
    /**
     * Persists a Keycloak event for the audit trail. Uses a savepoint to handle duplicate {@code
     * kcEventId} values idempotently (returns the existing record on constraint violation).
     *
    ...
    Stream<WebhookEventModel> getEventsByWebhookId(
            RealmModel realm, String webhookId, Integer first, Integer max);

    Stream<WebhookSendModel> getSendsByWebhook(
            RealmModel realm, String webhookId, Integer first, Integer max, Boolean success);

    Stream<WebhookSendModel> getFailedSendsSince(
            RealmModel realm, String webhookId, java.time.Instant since);

    // --- Send log ---

    /**
     * Records a webhook delivery attempt. If a send record already exists for the webhook-event
     * pair, it is updated in place (upsert semantics).
     *
