...
/**
 * Scheduled task that deletes old webhook events and send records based on per-realm retention
 * policies. Runs every 24 hours, scheduled by {@link
 * dev.montell.keycloak.listener.WebhookEventListenerProviderFactory}.
 *
 * <p>Retention periods are configured via realm attributes:
 *
 * <ul>
 *   <li>{@code _webhook.retention.events.days} — default {@value #DEFAULT_EVENT_DAYS}
 *   <li>{@code _webhook.retention.sends.days} — default {@value #DEFAULT_SEND_DAYS}
 * </ul>
 */
@JBossLog
public class RetentionCleanupTask implements ScheduledTask {
    ...
    public void run(KeycloakSession session) {
        ...
        for (RealmModel realm : realms) {
            int eventDays =
                    getRetentionDays(realm, "_webhook.retention.events.days", DEFAULT_EVENT_DAYS);
            int sendDays =
                    getRetentionDays(realm, "_webhook.retention.sends.days", DEFAULT_SEND_DAYS);
            ...
            if (eventsDeleted > 0 || sendsDeleted > 0) {
                log.infof(
                        "Retention cleanup realm=%s: deleted %d events, %d sends",
