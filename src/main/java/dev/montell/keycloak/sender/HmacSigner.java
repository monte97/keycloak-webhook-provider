...
/**
 * Computes HMAC signatures for webhook payload verification. Supports HmacSHA256 (default) and
 * HmacSHA1. The output is a lowercase hex-encoded digest suitable for use in the {@code
 * X-Keycloak-Signature} header.
 *
 * <p>Consumers should verify signatures using constant-time comparison ({@code
 * MessageDigest.isEqual} in Java, {@code hmac.compare_digest} in Python).
 */
    ...
    public static String sign(String payload, String secret, String algorithm) {
        try {
            ...
            throw new IllegalArgumentException(
                    "HMAC signing failed for algorithm: " + algorithm, e);
        }
    }
}
