...
/**
 * Matches event type strings against webhook subscription patterns. Supports wildcards ({@code *},
 * {@code access.*}, {@code admin.*}), exact match, and Java regex.
 *
...
public final class EventPatternMatcher {
    ...
    /**
     * Checks whether any pattern in the set matches the given event type. Pattern resolution order
     * (first match wins):
     *
     * <ol>
     *   <li>{@code *} — matches everything
     *   <li>{@code access.*} — prefix match for all user (access) events
     *   <li>{@code admin.*} — prefix match for all admin events
     *   <li>exact string equality
     *   <li>Java regex (via {@link String#matches}) — note: {@code .} is a regex wildcard here, not
     *       a literal dot. Patterns like {@code admin.USER-.*} work as expected.
     * </ol>
     */
